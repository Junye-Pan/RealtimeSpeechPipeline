...
// Config controls transport-kernel defaults for WebSocket adapter sessions.
type Config struct {
	DisconnectTimeout time.Duration
	DefaultDataClass  eventabi.PayloadClass
}

// Adapter implements shared transport contract behavior for WebSocket sessions.
type Adapter struct {
	cfg   Config
...
// NewAdapter creates a WebSocket adapter.
...
func (a *Adapter) Kind() apitransport.TransportKind {
	return apitransport.TransportWebSocket
}
...
func (a *Adapter) Open(ctx context.Context, bootstrap apitransport.SessionBootstrap) error {
	if bootstrap.TransportKind != apitransport.TransportWebSocket {
		return fmt.Errorf("websocket adapter requires transport_kind=websocket")
	}

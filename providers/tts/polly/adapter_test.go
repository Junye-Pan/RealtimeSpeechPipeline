...
func TestInvokeStreamSuccess(t *testing.T) {
	...
	chunks := 0
	completed := 0
	streamingAdapter, ok := adapter.(contracts.StreamingAdapter)
	if !ok {
		t.Fatalf("expected streaming adapter implementation")
	}

	outcome, err := streamingAdapter.InvokeStream(contracts.InvocationRequest{
		SessionID:            "sess-1",
		PipelineVersion:      "pipeline-v1",
		EventID:              "evt-1",
		ProviderInvocationID: "pvi-1",
	...
	if started != 1 || completed != 1 || chunks < 1 {
		t.Fatalf("unexpected streaming observer counts: start=%d chunks=%d complete=%d", started, chunks, completed)
	}
}

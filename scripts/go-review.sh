...
usage() {
  cat <<'EOF'
Usage: ./scripts/go-review.sh [run]

Runs Go review diagnostics: go vet, staticcheck (if available),
golangci-lint (if available), go build -race, govulncheck (if available).
EOF
}
...
run_action() {
  local ts report log
  ts="$(timestamp)"
  report="$REPORT_DIR/go-review_${ts}.md"
  log="$LOG_DIR/go-review_${ts}.log"
...
EOF
    echo "go-review: skipped (no Go files). Report: $report"
    return 0
  fi
...
EOF
  ...
  run_tool "go build -race" "go build -race ./..."
  if command -v govulncheck >/dev/null 2>&1; then
    run_tool "govulncheck" "govulncheck ./..."
  else
    statuses+=("govulncheck|SKIPPED (tool not found)")
  fi

  {
    echo "# Go Review Report"
    echo
    echo "Generated: $ts"
  ...
  echo "go-review: report written: $report"
  return "$exit_code"
}

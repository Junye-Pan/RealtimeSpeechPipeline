...
3. Pre-turn outcomes (`reject`, `defer`, `shed`, `stale_epoch_reject`, `deauthorized_drain`) do not emit `abort`/`close`.
4. OR-02 includes plan hash, determinism markers, authority markers, and failure-specific evidence from matrix row.
5. Replay of the run does not produce unexplained `PLAN_DIVERGENCE`, `OUTCOME_DIVERGENCE`, or `AUTHORITY_DIVERGENCE`.
6. `ORDERING_DIVERGENCE` is zero unless explicitly expected by fixture metadata.
